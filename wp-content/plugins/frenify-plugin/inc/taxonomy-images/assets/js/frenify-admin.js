jQuery(document).ready(function($) {
	...
	$(".frenify_upload_image").click(function(event) {
		upload_button 	= $(this);
		txt_box_id		= $(this).closest('.frenify-image-wrap').find('.frenify_image_url').attr('id');
		var frame;
		if (frenify.wp_version >= "3.5") {
			...
			frame.on( "select", function() {
				...
				if (upload_button.parent().prev().children().hasClass("tax_list")) {
					...
					if(upload_button.closest('.frenify-image-wrap').find('.frenify_image').length ){
						upload_button.closest('.frenify-image-wrap').find('.frenify_image').attr('src',attachment.attributes.sizes.thumbnail.url);
					}
	...
	$(".frenify_remove_image_button").click(function() {
		txt_box_id		= $(this).closest('.frenify-image-wrap').find('.frenify_image_url').attr('id');
		img_field		= $(this).closest('.frenify-image-wrap').find('.frenify_image');
		// on removal add placeholder image
		img_field.attr("src", frenify.featured_placeholder);
		$("#"+txt_box_id).val("");
	...
	$(".editinline").click(function() {	
		...
		if($(".inline-edit-col :input[name=\'taxonomy_featured_image\']").length) {
			if (thumb != frenify.featured_placeholder) {
		...
		if($(".inline-edit-col :input[name=\'taxonomy_cover_image\']").length) {
			if (cover_thumb != frenify.cover_placeholder) {
