...
$gsinglecols = '3';
if(isset($fotofly_fn_option['m_g_cols'])){
	$gsinglecols = $fotofly_fn_option['m_g_cols'];
}
$gsinglegutter = '80';
...
if(isset($fotofly_fn_option['gsingle_grid_type'])){
	$gsinglegridtype = $fotofly_fn_option['gsingle_grid_type'];
}

if(isset($_GET['temp'])){$gsinglelayout = $_GET['temp'];}
if(isset($_GET['col'])){$gsinglecols = $_GET['col'];}
if(isset($_GET['gutter'])){$gsinglegutter = $_GET['gutter'];}
if(isset($_GET['grid'])){$gsinglegridtype = $_GET['grid'];}

if($gsinglegridtype == 'square'){
	$gsinglethumb = fotofly_fn_callback_thumbs(1000,1000);
}else if($gsinglegridtype == 'portrait'){
	$gsinglethumb = fotofly_fn_callback_thumbs(800,970);
}else if($gsinglegridtype == 'landscape'){
	$gsinglethumb = fotofly_fn_callback_thumbs(840,570);
}
// Start Post
...
<div class="fotofly_fn_content_part" <?php echo esc_attr($fotofly_fn_page_spaces); ?>>
		...
		<div class="content_wrap">
			...
			<div class="fotofly_fn_gallery_single <?php echo esc_attr($gsinglelayout);?>">
				...
				<div class="fotofly_fn_gsingle_list <?php echo esc_attr($gsinglelayout);?>" data-cols="<?php echo esc_attr($gsinglecols);?>" data-gutter="<?php echo esc_attr($gsinglegutter);?>">
					<div>
							 ...
							 <?php
								...
								{   
									...
									foreach($fotofly_fn_gallery_images as $img){
										...
										$src 	= wp_get_attachment_image_src( $img['ID'], 'fotofly_fn_thumb-1000-1000' );
										$src 	= $src[0];
