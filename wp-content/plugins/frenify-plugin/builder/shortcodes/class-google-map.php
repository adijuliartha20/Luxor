...
class frenifySC_GoogleMap {
	...
	public function __construct() {
		...
		add_filter( 'fotofly_fn_attr_google-map-shortcode', array( $this, 'attr' ) );
	...
	function render( $args, $content = '' ) {
		...
		if( $address ) {
			...
			if( $map_style == 'theme' ) {
				...
				$animation = 'yes';
				$infobox = 'custom';
				$infobox_background_color = frenifyCore_Plugin::hex2rgb( $fotofly_fn_option['primary_color'] );
				$infobox_background_color = 'rgba(' . $infobox_background_color[0] . ', ' . $infobox_background_color[1] . ', ' . $infobox_background_color[2] . ', 0.8)';
				$overlay_color = $fotofly_fn_option['primary_color'];
				$brightness_level = frenifyCore_Plugin::calc_color_brightness( $fotofly_fn_option['primary_color'] );
			...
			if( $icon == 'theme' && $map_style == 'custom' ) {
				for( $i = 0; $i < $num_of_addresses; $i++ ) {
					$icon_array[$i] = plugins_url( 'images/fotofly_fn_map_marker.png', dirname( __FILE__ ) );				
				}
			}			
			...
			$cached_addresses = get_option( 'fotofly_fn_map_addresses' );
			...
			$map_id = uniqid( 'fotofly_fn_map_' ); // generate a unique ID for this map
			$this->map_id = $map_id;
			...
			<script type="text/javascript">
				...
				var markers = [];
				var counter = 0;
				function fotofly_fn_run_map_<?php echo $map_id ; ?>() {
					jQuery('#<?php echo $map_id ; ?>').fotofly_fn_maps({
						...
						overlay_color_hsl: <?php echo json_encode( frenifyCore_Plugin::rgb2hsl( $overlay_color ) ); ?>,
				...
				google.maps.event.addDomListener(window, 'load', fotofly_fn_run_map_<?php echo $map_id ; ?>);
			</script>
			<?php
			if( $defaults['id'] ) {
				$html = ob_get_clean() . sprintf( '<div id="%s"><div %s></div></div>', $defaults['id'], frenifyCore_Plugin::attributes( 'google-map-shortcode' ) );
			} else {
				$html = ob_get_clean() . sprintf( '<div %s></div>', frenifyCore_Plugin::attributes( 'google-map-shortcode' ) );
			}
	...
	function attr() {
		...
		$attr['class'] = 'shortcode-map frenify-google-map';
	...
	function get_coordinates( $address, $force_refresh = false ) {
		...
		$key = $fotofly_fn_option['google_console_api_key'];
		...
		if ( $force_refresh || 
			...
			if ( $response['response']['code'] == 200 ) {
				...
				if ( $data->status === 'OK' ) {
				  	...
				  	return __( 'No location found for the entered address.', 'frenify-core' );
				} elseif( $data->status === 'INVALID_REQUEST' ) {
				   	return __( 'Invalid request. Did you enter an address?', 'frenify-core' );
				} else {
					return __( 'Something went wrong while retrieving your map, please ensure you have entered the short code correctly.', 'frenify-core' );
				}

			} else {
			 	return __( 'Unable to contact Google API service.', 'frenify-core' );
			}
