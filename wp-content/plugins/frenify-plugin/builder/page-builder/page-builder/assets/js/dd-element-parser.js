...
( function($) { 
	...
	DdElementParser.generateHtml = function (model, settings_lvl) {
        ...
        if( model.get('subElements')[0].id == 'fotofly_fn_settings_lvl' ) {
        	lvl_exists = true;
		...
		jQuery.each( model.get('subElements') ,function( key, element ){
			...
			if( element['type'] == ADDMORE ) {
				...
				for (var i = 0; i < elementsCount; i++) {
					...
					output 		+= "<tr class='child-clone-row'><td><a href='#' class='frenify-expand-child'>" + model.get('name') + " Item " + (i + 1) + "<i class='frenifya-plus2'></i></a><div class='child-options' style='display: none;'>";
					...
					jQuery.each (element['elements'][i],function( innerKey, dynamic_element ) {
						...
						if( ChildElement['settings_lvl'] == 'child' && lvl_value == 'parent' ) {
							output 	+= "<div class='clearfix form-element-container frenify-element-child form-element-container-" + ChildElement['type'] + "' style='display: none;'><div class='name-description'>";
						} else {
							output 	+= "<div class='clearfix form-element-container frenify-element-child form-element-container-" + ChildElement['type'] + "'><div class='name-description'>";
						}
					...
					output 		+="<a class='child-clone-row-remove frenify-shortcodes-button' href='JavaScript:void(0)'>Remove</a>";
					output 		+= "</div></td></tr>";
				}

				output 			+= "<tr><td><a id='frenify-child-add' href='JavaScript:void(0)'>" + element['buttonText'] + "</a></td></tr>";
				output 			+= "</table>";
        ...
        if ($hasGroups) {
            output = $orig_output;
            var $tabID = 1;
            $menu = '<ul class="frenify-tabs-menu">';
            var $tabs = '<div class="frenify-tabs">';

            if ($group_html['Default'] != "") {
                $menu += '<li class="current"><a href="#tab-'+$tabID+'">Default</a></li>';
                $tabs += '<div id="tab-'+$tabID+'" class="frenify-tab-content" style="display:block;">'+$group_html['Default']+'</div>';
                $tabID++;
            }
            ...
            for ( var key in $group_html ) {
                if (key != "Default") {
                    ...
                    $tabs += '<div id="tab-'+$tabID+'" class="frenify-tab-content"'+$class+'>'+$group_html[key]+'</div>';
                    $tabID++;
                }
            }
            $menu += "</ul>";
	...
	DdElementParser.parseElementType = function (element) {
		...
		switch (element['type']) {
			...
			case COLOR:
				...
				output 		+= '<input type="text" class="text-field frenify-color-field" value="' + nl2br(element['value'],false) + '" id="' + element['id'] + '" name="' + element['id'] + '" size="50"/>';
			break;
			
			case GALLERY:
				output 		+= '<a href="'+element['id']+'" class="frenify-gallery-button frenify-shortcodes-button">Attach Images to Gallery</a>';
			break;
			...
			case UPLOAD:
				...
				output 		+= '<input type="hidden" class="frenify-form-text frenify-form-upload frenify-input" name="'+element['id'] +'" id="'+element['id'] +'" value="'+element['value'] +'" />' + "\n";
				output 		+= '<a href="'+element['id'] +'" class="frenifyb-upload-button '+button_class+'" data-upid="'+element['upid'] +'">'+button_text+'</a>';
				output		+= '<a href="'+element['id'] +'" style="'+edit_style+'" class="frenifyb-edit-button" data-upid="'+element['upid'] +'">Edit</a>';
				output 		+= '</div>';
