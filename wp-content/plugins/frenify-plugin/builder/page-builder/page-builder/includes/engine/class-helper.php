<?php
/**
 * frenifyHelper class with static methods
 */
class frenifyHelper  {
	/**
	...
	public static function OBJECT_TO_ARRAY( $object ) {
		...
		if ( is_array($object) ) {
			...
			*/
			return array_map("frenifyHelper::OBJECT_TO_ARRAY", $object);
		}
		else {
				// Return array
				return $object;
	 ...
	 * @return: data array
	 */
	public static function fotofly_fn_shortcodes_categories ( $taxonomy, $empty_choice = false, $empty_choice_label = 'Default' ) {
	 ...
	 * @return: data array
	 */
	 public static function fotofly_fn_create_dropdown_data ( $start, $end, $dataArray = array() ) {
		 
		 for ($i = $start; $i <= $end; $i++ ) {
			 $array['fotofly_fn_' . $i] = $i;
		 }
	...
	public static function GET_ICONS_LIST() {
		...
		$icons = new FAIterator(fotofly_fn_BUILDER_FA_PATH);
		$iconssArray = array();
		$output = '';
	...
	public static function get_layerslider_slides () {
		global $wpdb;
		$slides_array['fotofly_fn_0'] 	= 'Select a slider';
		// Table name
		...
		if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
			...
			if(isset($slides) && $slides){
				foreach($slides as $key => $val){
					$slides_array['fotofly_fn_'.$key] = 'LayerSlider #'.($key);
				}
			}
		}
	...
	public static function get_revslider_slides () {
		...
		global $wpdb;
		$revsliders['fotofly_fn_0'] 		= 'Select a slider';
			...
			if(function_exists('rev_slider_shortcode')) {
				...
				if($get_sliders) {
					foreach($get_sliders as $slider) {
						$revsliders['fotofly_fn_'.$slider->alias] = $slider->title;
					}
				}
			}
		}
	...
	public static function get_elasticslider_slides () {
		...
		$slides 			= get_terms('fotofly_fn_es_groups');
	...
	/**
	 * Returns array of theme frenify slider slide groups
	 * @param NULL
	 * @return array slide keys array
	 */
	 ...
	 * @return array
	 */
	public static function get_decimal_numbers () {
	 ...
	 * @return array
	 */
	public static function get_animation_type_data ( $defaults = true ) {
		if( $defaults ) {
			$types = 	array(
							'none' 					=> 'None',
							'bounce' 				=> 'Bounce',
