...
class frenifySC_SharingBox {
	...
	public function __construct() {
		...
		add_filter( 'fotofly_fn_attr_sharingbox-shortcode', array( $this, 'attr' ) );
		add_filter( 'fotofly_fn_attr_sharingbox-shortcode-tagline', array( $this, 'tagline_attr' ) );
		add_filter( 'fotofly_fn_attr_sharingbox-shortcode-social-networks', array( $this, 'social_networks_attr' ) );
		add_filter( 'fotofly_fn_attr_sharingbox-shortcode-icon', array( $this, 'icon_attr' ) );
	...
	function render( $args, $content = '') {
		...
		$defaults = frenifyCore_Plugin::set_shortcode_defaults(
			array(
				'class' 				=> '',
				'id' 					=> '',			
				'backgroundcolor' 		=> strtolower( $fotofly_fn_option['sharing_box_bg_color'] ),
				'description'			=> '',
				'icon_colors'			=> strtolower( $fotofly_fn_option['sharing_social_links_icon_color'] ),
				'box_colors'			=> strtolower( $fotofly_fn_option['sharing_social_links_box_color'] ),	
				'icons_boxed'			=> strtolower( $fotofly_fn_option['sharing_social_links_boxed'] ),
				'icons_boxed_radius'	=> strtolower( $fotofly_fn_option['sharing_social_links_boxed_radius'] ),		
				'link' 					=> '',
				'pinterest_image' 		=> '',
				'social_networks'		=> $this->get_theme_options_settings(),
				'tagline' 				=> __( 'Share This Story, Choose Your Platform!', 'frenify-core' ),
				'tagline_color'			=> strtolower( $fotofly_fn_option['sharing_box_tagline_text_color'] ),
				'title' 				=> '',
				'tooltip_placement'		=> strtolower( $fotofly_fn_option['sharing_social_links_tooltip_placement'] ),
			), $args
		);	
		...
		if( isset( $fotofly_fn_option['social_sorter'] ) && $fotofly_fn_option['social_sorter'] ) {
			$order = $fotofly_fn_option['social_sorter'];
			...
			if( isset( $ordered_array ) && $ordered_array && is_array( $ordered_array ) ) {
				...
				foreach( $ordered_array as $key => $field_order ) {
					...
					$find_the_field = $fotofly_fn_option['social_sorter_' . $field_order_number];
					$field_name = str_replace( '_link', '', $fotofly_fn_option['social_sorter_' . $field_order_number] );
		...
		for( $i = 0; $i < count( $social_networks ); $i++ ) {
			...
			$icons .= sprintf( '<a %s></a>', frenifyCore_Plugin::attributes( 'sharingbox-shortcode-icon', $icon_options ) );
		}

		$html = sprintf( '<div %s><h4 %s>%s</h4><div %s>%s</div></div>', frenifyCore_Plugin::attributes( 'sharingbox-shortcode' ), frenifyCore_Plugin::attributes( 'sharingbox-shortcode-tagline' ), 
						 $tagline, frenifyCore_Plugin::attributes( 'sharingbox-shortcode-social-networks' ), $icons );
	...
	function attr() {
		...
		$attr['class'] = 'share-box frenify-sharing-box';
...
//		}
	...
	function social_networks_attr() {
		...
		$attr['class'] = 'frenify-social-networks';
	...
	function icon_attr( $args ) {
		...
		$attr['class'] = sprintf( 'frenify-social-network-icon frenify-tooltip frenify-%s frenify-icon-%s', $args['social_network'], $args['social_network'] );	
		...
		if( $fotofly_fn_option['social_icons_new'] ) {
			$target = '_blank';
		} else {
			$target = '_self';
		}
		...
		if( $fotofly_fn_option['nofollow_social_links'] ) {
			$attr['rel'] = 'nofollow';
		}
	...
	function get_theme_options_settings() {
		...
		$social_media = '';
			...
			if( $fotofly_fn_option['sharing_facebook'] ) {
				$social_media .= 'facebook|';
			}

			if( $fotofly_fn_option['sharing_twitter'] ) {
				$social_media .= 'twitter|';
			}

			if( $fotofly_fn_option['sharing_linkedin'] ) {
				$social_media .= 'linkedin|';
			}
			
			if( $fotofly_fn_option['sharing_reddit'] ) {
				$social_media .= 'reddit|';
			}
			
			if( $fotofly_fn_option['sharing_tumblr'] ) {
				$social_media .= 'tumblr|';
			}
			
			if( $fotofly_fn_option['sharing_google'] ) {
				$social_media .= 'googleplus|';
			}
			
			if( $fotofly_fn_option['sharing_pinterest'] ) {
				$social_media .= 'pinterest|';
			}
			
			if( $fotofly_fn_option['sharing_vk'] ) {
				$social_media .= 'vk|';
			}			

			if( $fotofly_fn_option['sharing_email'] ) {
				$social_media .= 'mail|';
			}
