...
<?php 
	$width = 'auto';
	if(isset($fotofly_fn_option['p_single_carousel'])){
		$img_ratio 	= $fotofly_fn_option['p_single_carousel'];
	}else{
		$img_ratio 	= 'square';
		$width		= '736px';
	}
	...
	if(isset($_GET['ps_car_ratio'])){$img_ratio = $_GET['ps_car_ratio'];}
...
<div class="fotofly_fn_content_part" <?php echo esc_attr($fotofly_fn_page_spaces); ?>>
		...
		<div class="content_wrap">
			...
			<div class="fotofly_fn_portfolio_single carousel <?php echo esc_attr($autowidth);?>">
						...
						<?php
							...
							{   
								...
								foreach($fotofly_fn_portfolio_images as $img){
									...
									?>
								<?php 
									if($img_ratio == 'autowidth'){
										$width	= round($img['width']*500/$img['height']).'px';
									}
								?>
								<div class="item lightbox" style="width: <?php echo esc_attr($width);?>; height: 500px;"  data-src="<?php echo esc_attr($src2); ?>">
									<?php echo wp_kses_post($forLG);?>
									<div class="item_o_img" style="background-image: url(<?php echo esc_url($src); ?>)"></div>
