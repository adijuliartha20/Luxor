...
if( ! class_exists( 'fotofly_fn_Core_Custom_Templates' ) ) {

	class fotofly_fn_Core_Custom_Templates {
		...
		public function get_custom_templates () {
			...
			$content 		= '<div id="custom_templates_wrapper">
								...
								<h1 class="templates_heading">'.__('Save Template', 'frenify-core').'</h1>
								<div class="save_templates_here">
								<a class="templates_selection" id="fotofly_fn_save_custom_tpl" 
								href="JavaScript:void(0)">'.__('Save Layout As Template', 'frenify-core').'</a>
								</div>
								</div>
								<div id="custom_templates_right">
								<h1 class="templates_heading">'.__('Load Template', 'frenify-core').'</h1>';
			//get templates data
			$templates 		= get_option( 'fotofly_fn_custom_templates' );
				...
				for( $i = 0; $i < $columns; $i++) {
					...
					foreach( $templates as $key => $value ) {
						...
						$content.= ' <div class="hidden_overlay">
										<a href="JavaScript:void(0)" data-id="'.$key.'"  class="fotofly_fn_load_template">'.__('LOAD', 'frenify-core').'</a>
										<a href="JavaScript:void(0)" data-id="'.$key.'"  class="fotofly_fn_delete_template">'.__('DELETE', 'frenify-core').'</a>
									 </div>';
						$content.= ' <a class="fotofly_fn_custom_template templates_selection" href="JavaScript:void(0)">'.$key.'</a>';
						$content.= ' </div>';
		...
		public function get_single_template () {
			...
			$custom_templates = get_option( 'fotofly_fn_custom_templates' );
		...
		public function save_single_template () {
			...
			if ( $custom_templates === false ) {
				...
				$response 							= add_option( 'fotofly_fn_custom_templates', $custom_templates, '', 'no' );
			} else {
				
				$custom_templates[ stripslashes($_POST['name']) ] = $model;
				
				$response 							= update_option( 'fotofly_fn_custom_templates',  $custom_templates );
