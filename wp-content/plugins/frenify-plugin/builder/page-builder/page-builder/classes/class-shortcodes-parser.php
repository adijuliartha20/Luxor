...
if( ! class_exists( 'fotofly_fn_Core_Shortcodes_Parser' ) ) {

	class fotofly_fn_Core_Shortcodes_Parser {
		...
		public static function check_builder_elements ( $content ) {
			...
			self::$meta_content = get_post_meta( $post->ID, "fotofly_fn_builder_content" , TRUE); //get builder contents 
			if( !empty( self::$meta_content ) ) { //if page built through frenify builder
				$short_codes_content = fotofly_fn_Core_Shortcodes_Parser::parse_column_options( );  // let the magic begin
			...
			foreach ( self::$meta_content as $element ) { //traverse elements
				...
				switch ($css_class) { //switch on unique element
					...
					case 'grid_two' :
						...
						$short_codes.= '[one_half '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						//check if child elements exist. Then parse elements one by one
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_three' :
						...
						$short_codes.= '[one_third '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_four' :
						...
						$short_codes.= '[one_fourth '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_five' :
						...
						$short_codes.= '[one_fifth '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_two_fifth' :
						...
						$short_codes.= '[two_fifth '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_three_fifth' :
						...
						$short_codes.= '[three_fifth '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_four_fifth' :
						...
						$short_codes.= '[four_fifth '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_six' :
						...
						$short_codes.= '[one_sixth '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_five_sixth' :
						...
						$short_codes.= '[five_sixth '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_three_fourth' :
						...
						$short_codes.= '[three_fourth '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					case 'grid_two_third' :
						...
						$short_codes.= '[two_third '.fotofly_fn_Core_Shortcodes_Parser::prepare_column_attr( $element['subElements'] ).']';
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
						$short_codes.= '[/two_third]';
						
					break;
					
					case 'fotofly_fn_full_width' :
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::build_full_width_container_Shortcode( $element['subElements'] );
						$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_column_element( $element );
					...
					default: //default case. For elements without layout column
						if ( !isset ( $element['parentId'] ) ) { //if element does not have any parent (column element)
							...
							$short_codes.= fotofly_fn_Core_Shortcodes_Parser::parse_builder_elements( $element );
						}
				}
			}
		...
		private static function parse_column_element ( $element ) {
			...
			if ( $child_elements > 0 ) {
				$colum_elements = fotofly_fn_Core_Shortcodes_Parser::parse_child_elements( $element );
			}
		...
		private static function parse_child_elements ($element ) {
			...
			foreach ($element['childrenId'] as $child) {
				...
				$builder_element = fotofly_fn_Core_Shortcodes_Parser::search_child_element ( $child['id'] );
				
				if ($builder_element != false) {
					
					$builder_element_shortcode.= fotofly_fn_Core_Shortcodes_Parser::parse_builder_elements($builder_element);
				}
		...
		private static function parse_builder_elements ( $element ) {
			...
			switch ($element['php_class']) { //switch on unique element
				...
				case 'fotofly_fn_AlertBox':
					return fotofly_fn_Core_Shortcodes_Parser::build_alert_Shortcode( $element['subElements'] );
				break;
				
				case 'fotofly_fn_WpBlog':
					return fotofly_fn_Core_Shortcodes_Parser::build_blog_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_ButtonBlock':
					return fotofly_fn_Core_Shortcodes_Parser::build_button_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_CheckList' :
					return fotofly_fn_Core_Shortcodes_Parser::build_checklist_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_ClientSlider' : 
					return fotofly_fn_Core_Shortcodes_Parser::build_client_slider_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_ContentBoxes' : 
					return fotofly_fn_Core_Shortcodes_Parser::build_content_box_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_CounterCircle' : 
					return fotofly_fn_Core_Shortcodes_Parser::build_counter_circle_Shortcode( $element['subElements'] ) ;
				break; 
				
				case 'fotofly_fn_CounterBox' :
					return fotofly_fn_Core_Shortcodes_Parser::build_counter_box_Shortcode( $element['subElements'] ) ;
				break;
				
				/*case 'fotofly_fn_DropCap' :
					return fotofly_fn_Core_Shortcodes_Parser::build_dropcap_Shortcode( $element['subElements'] ) ;
				break;*/
				
				case 'fotofly_fn_PostSlider' :
					return fotofly_fn_Core_Shortcodes_Parser::build_post_slider_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_FlipBoxes' :
					return fotofly_fn_Core_Shortcodes_Parser::build_flip_boxes_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_FontAwesome' :
					return fotofly_fn_Core_Shortcodes_Parser::build_font_awesome_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_FullWidthContainer' :
					return fotofly_fn_Core_Shortcodes_Parser::build_full_width_container_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_GoogleMap' :
					return fotofly_fn_Core_Shortcodes_Parser::build_google_map_Shortcode( $element['subElements'] ) ; 
				break;
				
				/*case 'fotofly_fn_HighLight' :
					return fotofly_fn_Core_Shortcodes_Parser::build_highlight_Shortcode( $element['subElements'] ) ; 
				break;*/
				
				case 'fotofly_fn_ImageFrame' :
					return fotofly_fn_Core_Shortcodes_Parser::build_image_frame_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_ImageCarousel' :
					return fotofly_fn_Core_Shortcodes_Parser::build_image_carousel_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_Intro' :
					return fotofly_fn_Core_Shortcodes_Parser::build_intro_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_LightBox' :
					return fotofly_fn_Core_Shortcodes_Parser::build_light_box_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_LayerSlider' :
					return fotofly_fn_Core_Shortcodes_Parser::build_layer_slider_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_MenuAnchor' :
					return fotofly_fn_Core_Shortcodes_Parser::build_menu_anchor_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_Modal' :
					return fotofly_fn_Core_Shortcodes_Parser::build_modal_Shortcode( $element['subElements'] ) ; 
				break;
				
				/*case 'fotofly_fn_Modal_Link' :
					return fotofly_fn_Core_Shortcodes_Parser::build_modal_link_Shortcode( $element['subElements'] ) ; 
				break;*/
				
				case 'fotofly_fn_Person' :
					return fotofly_fn_Core_Shortcodes_Parser::build_person_Shortcode( $element['subElements'] ) ; 
				break;
					
				case 'fotofly_fn_AboutMe' :
					return fotofly_fn_Core_Shortcodes_Parser::build_about_me_Shortcode( $element['subElements'] ) ; 
				break;
				
				/*case 'fotofly_fn_Popover' :
					return fotofly_fn_Core_Shortcodes_Parser::build_popover_Shortcode( $element['subElements'] ) ; 
				break;*/
				
				case 'fotofly_fn_PricingTable' :
					return fotofly_fn_Core_Shortcodes_Parser::build_pricing_table_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_ProgressBar' :
					return fotofly_fn_Core_Shortcodes_Parser::build_progress_bar_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_RecentPosts' :
					return fotofly_fn_Core_Shortcodes_Parser::build_recent_posts_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_GalleryBlock' : 
					return fotofly_fn_Core_Shortcodes_Parser::build_gallery_block_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_RevolutionSlider' :
					return fotofly_fn_Core_Shortcodes_Parser::build_rev_slider_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_SectionSeparator' :
					return fotofly_fn_Core_Shortcodes_Parser::build_section_separator_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_Separator' :
					return fotofly_fn_Core_Shortcodes_Parser::build_separator_Shortcode( $element['subElements'] ) ; 
				break; 
				
				case 'fotofly_fn_SharingBox' :
					return fotofly_fn_Core_Shortcodes_Parser::build_sharing_box_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_Slider' :
					return fotofly_fn_Core_Shortcodes_Parser::build_slider_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_SoundCloud' :
					return fotofly_fn_Core_Shortcodes_Parser::build_soundcloud_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_SocialLinks' :
					return fotofly_fn_Core_Shortcodes_Parser::build_social_links_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_Tabs' :
					return fotofly_fn_Core_Shortcodes_Parser::build_tabs_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_Table' :
					return fotofly_fn_Core_Shortcodes_Parser::build_table_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_TaglineBox' :
					return fotofly_fn_Core_Shortcodes_Parser::build_tagline_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_Testimonial' :
					return fotofly_fn_Core_Shortcodes_Parser::build_testimonial_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_TextBlock' :
					return fotofly_fn_Core_Shortcodes_Parser::build_text_block_Shortcode( $element['subElements'] ) ; 
				break;
				
				case 'fotofly_fn_CustomTitle':
					return fotofly_fn_Core_Shortcodes_Parser::build_title_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_Accordion' :
					return fotofly_fn_Core_Shortcodes_Parser::build_accordion_Shortcode( $element['subElements'] ) ;
				break;
				
				/*case 'fotofly_fn_Tooltip':
					return fotofly_fn_Core_Shortcodes_Parser::build_tooltip_Shortcode( $element['subElements'] ) ;
				break;*/
				
				case 'fotofly_fn_Vimeo':
					return fotofly_fn_Core_Shortcodes_Parser::build_vimeo_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_WooFeatured' :
					return fotofly_fn_Core_Shortcodes_Parser::build_woo_featured_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_WooCarousel' :
					return fotofly_fn_Core_Shortcodes_Parser::build_woo_carousel_Shortcode( $element['subElements'] ) ;
				break;
				
				case 'fotofly_fn_WooShortcodes' :
					return fotofly_fn_Core_Shortcodes_Parser::build_woo_shortcodes( $element['subElements'] ) ;
				break;

				case 'fotofly_fn_Youtube':
					return fotofly_fn_Core_Shortcodes_Parser::build_youtube_Shortcode( $element['subElements'] ) ;
				break;
		...
		**/
		 ...
		 **/
		private static function build_about_me_Shortcode ( $args ) {
			
			$shortcode_data = '[about_me_fn ';
			$shortcode_data.= ' skin="'+args[0].value+'"';
			$shortcode_data.= ' image_position="'+args[1].value+'"';
			$shortcode_data.= ' image="'+args[2].value+'"';
			$shortcode_data.= ' name="'+args[3].value+'"';
			$shortcode_data.= ' occupation="'+args[4].value+'"';
			$shortcode_data.= ' info_title="'+args[5].value+'"';

			$shortcode_data.= ' info_button_text="'+args[7].value+'"';
			$shortcode_data.= ' info_button_url="'+args[8].value+'"';
			$shortcode_data.= ' info_button_target="'+args[9].value+'"';

			// social icons
			$shortcode_data.= ' email="'+args[10].value+'"';
			$shortcode_data.= ' facebook="'+args[11].value+'"';
			$shortcode_data.= ' twitter="'+args[12].value+'"';
			$shortcode_data.= ' instagram="'+args[13].value+'"';
			$shortcode_data.= ' google="'+args[14].value+'"';
			$shortcode_data.= ' linkedin="'+args[15].value+'"';
			$shortcode_data.= ' vimeo="'+args[16].value+'"';
			$shortcode_data.= ' youtube="'+args[17].value+'"';
			$shortcode_data.= ' flickr="'+args[18].value+'"';
			$shortcode_data.= ' skype="'+args[19].value+'"';
			$shortcode_data.= ' tumblr="'+args[20].value+'"';
			$shortcode_data.= ' dribbble="'+args[21].value+'"';
			$shortcode_data.= ' vkontakte="'+args[22].value+'"';
			$shortcode_data.= ' 500px="'+args[23].value+'"';
			$shortcode_data.= ' odnoklassniki="'+args[24].value+'"';
			$shortcode_data.= ' pinterest="'+args[25].value+'"';

			$shortcode_data.= ' margin_top="'+args[26].value+'"';
			$shortcode_data.= ' margin_bottom="'+args[27].value+'"';
			$shortcode_data.= ' class="'+args[28].value+'"';
			$shortcode_data.= ' id="'+args[29].value+'"]';
			$shortcode_data.=   args[6].value;
			$shortcode_data.= '[/about_me_fn]';
		...
		private static function build_tabs_Shortcode ( $args ) {
			...
			$shortcode_data = ' [fotofly_fn_tabs ';
			$shortcode_data.= ' layout="'.$args[0]['value'].'" ';
		...
		private static function build_testimonial_Shortcode ( $args ) {
			...
			$shortcode_data.= ' image="'.$args[0]['value'].'" ';
			$shortcode_data.= ' design="'.$args[1]['value'].'" ';
			$shortcode_data.= ' backgroundcolor="'.$args[2]['value'].'" ';
			$shortcode_data.= ' textcolor="'.$args[3]['value'].'" ';
			$shortcode_data.= ' class="'.$args[4]['value'].'" ';
			$shortcode_data.= ' id="'.$args[5]['value'].'"] ';
