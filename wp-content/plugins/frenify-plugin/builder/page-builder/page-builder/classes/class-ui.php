...
if( ! class_exists( 'fotofly_fn_Core_PageBuilder_UI' ) ) {

	class fotofly_fn_Core_PageBuilder_UI {
		...
		public function wrap_default_editor() {
			...
			if ( in_array( $screen->id, $allowed_screens) ) {
				...
				$pagebuilder_label 	= __( 'Frenify Page Builder', 'frenify-core' );
				$default_label  	= __( 'Default Editor', 'frenify-core' );
				$status		 	= get_post_meta($post_ID, '_frenifyPageBuilderStatus', true);
				echo '<a id="frenify-pb-switch-button" href="javascript:void(0);" class="button-primary" data-active-button="' . $default_label . '" data-inactive-button="' . $pagebuilder_label . '">' . $pagebuilder_label . '</a>';
		...
		public function close_default_editor_wrap() {
			...
			if ( in_array( $screen->id, $allowed_screens) ) {
				...
				echo '<textarea class="frenify-builder-pagecontent">' . $content . '</textarea>';
