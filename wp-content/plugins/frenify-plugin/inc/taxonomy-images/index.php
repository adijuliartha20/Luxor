...
if ( ! class_exists( 'EASY_TAX_IMAGES' ) ) :
	...
	final class EASY_TAX_IMAGES {
		...
		public $prefix 		= 'frenify_';
		
		public $text_domain	= 'fotofly';
		...
		private function hooks() {
			...
			// hooks browser menu
			add_action('admin_menu', array($this,'easy_tax_images_menu'));
		...
		public function easy_tax_images_menu() {
			 add_options_page(  
			  	__( 'Taxonomy Images', $this->text_domain ),
			  	__( 'Taxonomy Images', $this->text_domain ), 
			  	'manage_options',
			  	$this->prefix.'tax_images',
			  	array($this,'frenify_callback')
		  	); 
		}
		...
		public function admin_init() {
			...
			if( isset($_POST['option_page']) && $_POST['option_page'] == 'easy-tax-images-settings'  ) {
				...
				if(!empty($_POST['frenify_options'])) {
				
					update_option('frenify_options',$_POST['frenify_options']);
				}
			...
			if (is_array($taxes)) {
				...
				$frenify_options = get_option('frenify_options');
				$excluded_taxonomies_featured   = !empty($frenify_options['excluded_taxonomies_featured'])? $frenify_options['excluded_taxonomies_featured'] : array();
                $excluded_taxonomies_cover      = !empty($frenify_options['excluded_taxonomies_cover'])? $frenify_options['excluded_taxonomies_cover'] : array();
		...
		function add_taxonomy_field_featured() {
			...
			echo '
			<div class="form-field frenify-image-wrap">
				<label for="taxonomy_featured_image">' . __('Image', $this->text_domain) . '</label>
				<img class="frenify_image" src=""/><br/>
				<input type="text" class="frenify_image_url" name="taxonomy_featured_image" id="taxonomy_featured_image" value="" />
				<br/>
				<button class="frenify_upload_image button">' . __('Upload/Add image', $this->text_domain) . '</button>
			</div>';
        ...
        function add_taxonomy_field_cover() {
			...
			<div class="form-field frenify-image-wrap">
				...
				<img class="frenify_image" src=""/><br/>
				<input type="text" class="frenify_image_url" name="taxonomy_cover_image" id="taxonomy_cover_image" value="" />
				<br/>
				<button class="frenify_upload_image button">' . __('Upload/Add image', $this->text_domain) . '</button>
			</div>';
        }
		...
		function edit_taxonomy_field_featured($taxonomy) {
			...
			<tr class="form-field">
				...
				<td class="frenify-image-wrap">
					<img class="frenify_image" src="' . $this->taxonomy_image_url( $taxonomy->term_id, 'medium', TRUE ) . '"/><br/>
					<input type="text" class="frenify_image_url" name="taxonomy_featured_image" id="taxonomy_featured_image" value="'.$image_url.'" /><br />
					<button class="frenify_upload_image button button">' . __('Upload/Add image', $this->text_domain) . '</button>
					<button class="frenify_remove_image_button button">' . __('Remove image', $this->text_domain) . '</button>
				</td>
			</tr>';
        ...
        function edit_taxonomy_field_cover($taxonomy) {
			...
			<tr class="form-field">
				...
				<td class="frenify-image-wrap">
					<img class="frenify_image frenify_image" src="' . $this->taxonomy_image_url( $taxonomy->term_id, 'medium', TRUE ) . '"/><br/>
					<input type="text" class="frenify_image_url" name="taxonomy_cover_image" id="taxonomy_cover_image" value="'.$image_url.'" /><br />
					<button class="frenify_upload_image button button">' . __('Upload/Add image', $this->text_domain) . '</button>
					<button class="frenify_remove_image_button button">' . __('Remove image', $this->text_domain) . '</button>
				</td>
			</tr>';
		...
		function placeholder($type='taxonomy_featured_image') {
			...
			if($type == 'taxonomy_featured_image') {
				return apply_filters('frenify_default_taxonomy_featured_image',$featured);
			} elseif($type == 'taxonomy_cover_image') {
                return apply_filters('frenify_default_taxonomy_featured_image',$cover);
			}
		}
		...
		function quick_edit_custom_box($column_name, $screen, $name) {
			if ($column_name == 'thumb') 
				echo '
				<fieldset>
					<div class="thumb inline-edit-col frenify-image-wrap">
						<label>
							<span class="title"><img src="" alt="img"/></span>
							<span class="input-text-wrap">
								<input type="text" name="taxonomy_featured_image" value="" class="tax_list" />
							</span>
							<span class="input-text-wrap">
								<button class="frenify_upload_image button">' . __('Add image', $this->text_domain) . '</button>
								<button class="frenify_remove_image_button button">' . __('Remove image', $this->text_domain) . '</button>
							</span>
						</label>
					</div>
			...
			echo '
			<fieldset>
				<div class="cover_thumb inline-edit-col frenify-image-wrap">
					<label>
						<span class="title"><img src="" alt="img"/></span>
						<span class="input-text-wrap">
		...
		public function setup_constants() {		
			...
			// Plugin File
			if ( ! defined( 'FRENIFY_PLUGIN_FILE' ) ) {
				define( 'FRENIFY_PLUGIN_FILE', __FILE__ );
			}
			
			// Plugin Folder URL
			if ( ! defined( 'FRENIFY_PLUGIN_URL' ) ) {
				define( 'FRENIFY_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
			}
			
			// Plugin Folder Path
			if ( ! defined( 'FRENIFY_PLUGIN_PATH' ) ) {
				define( 'FRENIFY_PLUGIN_PATH', plugin_dir_path( __FILE__ ) );
			}
			
			// Plugin dir path
			if ( ! defined( 'FRENIFY_PLUGIN_DIR' ) ) {
				define( 'FRENIFY_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
			}
		...
		function common_includes() {
			do_action('frenify_common_includes');
		}
		...
		function admin_includes() {
			do_action('frenify_admin_includes');
		}
		...
		function ajax_includes() {
			do_action('frenify_ajax_includes');
		}
		...
		function frontend_includes() {
			do_action('frenify_frontend_includes');
		}
		 ...
		 * @return void
		 */
		function frenify_callback() {
			include_once(FRENIFY_PLUGIN_DIR.'lib/admin-page.php');
		}
		...
		function admin_enqueue_scripts() {
			...
			wp_enqueue_style( 'frenify-admin-stylesheet', FRENIFY_PLUGIN_URL . 'assets/css/frenify-admin.css', array(), $this->version);
			wp_register_script( 'frenify-admin-script', FRENIFY_PLUGIN_URL . 'assets/js/frenify-admin.js', array('jquery'), $this->version);
			wp_localize_script( 'frenify-admin-script', 'frenify', $js_vars );
			wp_enqueue_script( 'frenify-admin-script');
		}
...
function frenify_taxonomy_image($term_id = NULL, $size = 'medium', $attr = NULL, $echo = TRUE,$key = 'taxonomy_featured_image') {
    WPFRENIFY()->display_images($term_id,$size,$attr,$echo,$key);
}
...
function f_display_image_url($term_id = NULL, $key = 'taxonomy_featured_image') {
