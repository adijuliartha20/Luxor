...
(function( $ ) {
        ...
        element.draggable(
            {
                ...
                start: function( event, ui ) {
                    ...
                    //remove all previous frenify-hover-active classes
                    $( '.frenify-hover-active' ).removeClass( 'frenify-hover-active' );
                    ...
                    $( ui.helper ).addClass( 'frenify-dragging-element' );
                ...
                stop: function( event, ui ) {
                    ...
                    //remove frenify-hover-active class from all elements
                    $( '.frenify-hover-active' ).removeClass( 'frenify-hover-active' );
                    ...
                    if ( window.oldParent.length > 9 ) {
                        ...
                        if ( phpClass == 'fotofly_fn_FullWidthContainer' && cElement.get( 'childrenId' ).length > 0 ) {
        ...
        element.droppable(
            {
                ...
                over: function( event, ui ) {
                    ...
                    if ( DdHelper.isDropingAllowed( ui.helper, dropable ) ) {
                        ...
                        dropable.parent().parent().removeClass( 'frenify-hover-active' );
                        dropable.addClass( 'frenify-hover-active' );
                    }
                },
                ...
                out: function( event, ui ) {
                    ...
                    $( this ).removeClass( 'frenify-hover-active' );
                },
                ...
                drop: function( event, ui ) {
                    ...
                    if ( !dropable.is( '.frenify-hover-active' ) ) return false;
                    ...
                    if ( elementId != 'editor' ) {
                        ...
                        if ( parentId ) {
                            ...
                            if ( parent.get( 'php_class' ) == 'fotofly_fn_FullWidthContainer' && method == 'prepend' ) {
                    ...
                    if ( phpClass == 'fotofly_fn_FullWidthContainer' && cElement.get( 'childrenId' ).length > 0 ) {
                        /*//turn off editor tracking first
                         ...
                         var elements = frenifyHistoryManager.getAllElementsData();
        ...
        var $dialog = $( '#dialog_form' ).dialog(
            {
                ...
                draggable: false,
                modal: true,
                dialogClass: 'frenifyb-dialog-form',
                buttons: {
                    "SAVE": function() {
                        tinyMCE.triggerSave();
                ...
                open: function( event, ui ) {
                    $( this ).parent().find( '.ui-dialog-titlebar' ).find( '.frenify-tabs-menu' ).remove();
					...
					// 
					DdHelper.fn_id_to_class_transfer();
	...
	DdHelper.fn_id_to_class_transfer = function(){
		
		var myParent = $('.form-element-container');
		myParent.each(function(){
			var element = $(this);
			var myItem	= element.find('.element-type > *:last-child');
			var myID2	= element.find('.element-type input').attr('id');
			var myID 	= myItem.attr('id');
			
			if(element.hasClass('form-element-container-upload')){
				element.addClass('wrapper_' + myID2);
			}else{
				element.addClass('wrapper_' + myID);
			}
		});
		
		// this function only for shortcode PORTFOLIO CUSTOM
		setTimeout(function(){
			var myselect 				= jQuery('select#fotofly_fn_fn_post_type');
			var catSlugOfPortfolio 		= jQuery('.form-element-container.wrapper_fotofly_fn_cat_slug');
			var catExcludeOfPortfolio 	= jQuery('.form-element-container.wrapper_fotofly_fn_exclude_cats');
			var catSlugOfGallery 		= jQuery('.form-element-container.wrapper_fotofly_fn_cat_slug_gallery');
			var catExcludeOfGallery 	= jQuery('.form-element-container.wrapper_fotofly_fn_exclude_cats_gallery');
			
			catSlugOfGallery.hide();
			catExcludeOfGallery.hide();
			catSlugOfPortfolio.hide();
			catExcludeOfPortfolio.hide();
			
			myselect.on('change', function(){
				var chosed = jQuery(this).children('option:selected').val();
				if(chosed === 'gallery'){
					catSlugOfGallery.slideDown();catExcludeOfGallery.slideDown();
					catSlugOfPortfolio.slideUp();catExcludeOfPortfolio.slideUp();
				}else if((chosed === 'portfolio')){
					catSlugOfGallery.slideUp();catExcludeOfGallery.slideUp();
					catSlugOfPortfolio.slideDown();catExcludeOfPortfolio.slideDown();
				}
			});
			myselect.triggerHandler("change");
		},1);
		// this function only for shortcode CUSTOMPOST CATEGORY FOLDER
		setTimeout(function(){
			var myselect2 				= jQuery('select#fotofly_fn_fnposttype');
			var catSlugOfPortfolio 		= jQuery('.form-element-container.wrapper_fotofly_fn_catslug_folder');
			var catSlugOfGallery 		= jQuery('.form-element-container.wrapper_fotofly_fn_catslug_folder_gallery');
			...
			myselect2.on('change', function(){
				var chosed = jQuery(this).children('option:selected').val();
				if(chosed === 'gallery'){
					catSlugOfGallery.slideDown();catSlugOfPortfolio.slideUp();
				}else if((chosed === 'portfolio')){
					catSlugOfGallery.slideUp();catSlugOfPortfolio.slideDown();
				}
			});
			myselect2.triggerHandler("change");
		},1);
		// this function only for shortcode BUTTONFN (main button)
		setTimeout(function(){
			var bgSelect 				= jQuery('select#fotofly_fn_btnbgtype');
			var buttonTypeSelect 		= jQuery('select#fotofly_fn_btntype');
			var hoverSelect 			= jQuery('select#fotofly_fn_btnhoveranim');
			var bgColor		 			= jQuery('.form-element-container.wrapper_fotofly_fn_btnbgcol');
			var gradDirection 			= jQuery('.form-element-container.wrapper_fotofly_fn_btngrddir');
			var gradStartColor	 		= jQuery('.form-element-container.wrapper_fotofly_fn_btngradstartcol');
			var gradEndColor	 		= jQuery('.form-element-container.wrapper_fotofly_fn_btngradendcol');
			var arrColor		 		= jQuery('.form-element-container.wrapper_fotofly_fn_btnarrcol');
			var textColorHover		 	= jQuery('.form-element-container.wrapper_fotofly_fn_btntextcol_hover');
			var bgColorHover		 	= jQuery('.form-element-container.wrapper_fotofly_fn_btnbgcol_hover');
			var borderColorHover		= jQuery('.form-element-container.wrapper_fotofly_fn_btnborcol_hover');
			var gradStartColorHover		= jQuery('.form-element-container.wrapper_fotofly_fn_btngradstartcol_hover');
			var gradEndColorHover		= jQuery('.form-element-container.wrapper_fotofly_fn_btngradendcol_hover');
			var arrColorHover			= jQuery('.form-element-container.wrapper_fotofly_fn_btnarrcol_hover');
			
			gradDirection.hide();gradStartColor.hide();gradEndColor.hide();gradStartColorHover.hide();			gradStartColorHover.hide();gradEndColorHover.hide();
			
			bgSelect.on('change', function(){
				var chosed = jQuery(this).children('option:selected').val();
				if(chosed === 'none'){
					bgColor.slideUp();bgColorHover.slideUp();
					gradStartColorHover.slideUp();gradEndColorHover.slideUp();
					gradDirection.slideUp();gradStartColor.slideUp();gradEndColor.slideUp();
				}else if(chosed === 'gradient'){
					gradStartColorHover.slideDown();gradEndColorHover.slideDown();
					gradDirection.slideDown();gradStartColor.slideDown();gradEndColor.slideDown();
				}else if(chosed === 'color'){
					bgColor.slideDown();bgColorHover.slideDown();
					gradStartColorHover.slideUp();gradEndColorHover.slideUp();
					gradDirection.slideUp();gradStartColor.slideUp();gradEndColor.slideUp();
				}
			});
			bgSelect.triggerHandler("change");
			
			hoverSelect.on('change', function(){
				var chosed = jQuery(this).children('option:selected').val();
				if(chosed === 'disable'){
					textColorHover.slideUp();bgColorHover.slideUp();
					borderColorHover.slideUp();gradStartColorHover.slideUp();
					gradEndColorHover.slideUp();arrColorHover.slideUp();
				}else{
					bgColorHover.slideDown();
					borderColorHover.slideDown();
				}
			});
			hoverSelect.triggerHandler("change");
			
			buttonTypeSelect.on('change', function(){
				var chosed = jQuery(this).children('option:selected').val();
				if(chosed === 'simple'){
					arrColor.slideUp();
					arrColorHover.slideUp();
				}else{
					arrColor.slideDown();
					arrColorHover.slideDown();
				}
			});
			buttonTypeSelect.triggerHandler("change");
		},1);
		// this function only for shortcode: flipbox-fn
		setTimeout(function(){
			var flipBgType1		= jQuery('select#fotofly_fn_flip_bgtype_front');
			var bgColor1 		= jQuery('.form-element-container.wrapper_fotofly_fn_flip_bgcol_front');
			var bgGrDeg1 		= jQuery('.form-element-container.wrapper_fotofly_fn_flip_grdeg_front');
			var bgGrStart1 		= jQuery('.form-element-container.wrapper_fotofly_fn_flip_grstart_front');
			var bgGrEnd1 		= jQuery('.form-element-container.wrapper_fotofly_fn_flip_grend_front');
			var bgImg1	 		= jQuery('.form-element-container.wrapper_fotofly_fn_flip_bgimg_front');
			...
			flipBgType1.on('change', function(){
				var chosed = jQuery(this).children('option:selected').val();
				if(chosed === 'color'){
					bgGrStart1.slideUp();bgGrEnd1.slideUp();bgImg1.slideUp();bgGrDeg1.slideUp();
					bgColor1.slideDown();
				}else if((chosed === 'gradient')){
					bgImg1.slideUp();
					bgGrStart1.slideDown();bgGrEnd1.slideDown();bgColor1.slideDown();bgGrDeg1.slideDown();
				}else if((chosed === 'image')){
					bgImg1.slideDown();
					bgGrStart1.slideUp();bgGrEnd1.slideUp();bgColor1.slideUp();bgGrDeg1.slideUp();
				}
			});
			flipBgType1.triggerHandler("change");
		},1);
		setTimeout(function(){
			var flipBgType2		= jQuery('select#fotofly_fn_flip_bgtype_back');
			var bgColor2 		= jQuery('.form-element-container.wrapper_fotofly_fn_flip_bgcol_back');
			var bgGrDeg2 		= jQuery('.form-element-container.wrapper_fotofly_fn_flip_grdeg_back');
			var bgGrStart2 		= jQuery('.form-element-container.wrapper_fotofly_fn_flip_grstart_back');
			var bgGrEnd2 		= jQuery('.form-element-container.wrapper_fotofly_fn_flip_grend_back');
			var bgImg2	 		= jQuery('.form-element-container.wrapper_fotofly_fn_flip_bgimg_back');
			...
			flipBgType2.on('change', function(){
				var chosed = jQuery(this).children('option:selected').val();
				if(chosed === 'color'){
					bgGrStart2.slideUp();bgGrEnd2.slideUp();bgImg2.slideUp();bgGrDeg2.slideUp();
					bgColor2.slideDown();
				}else if((chosed === 'gradient')){
					bgImg2.slideUp();
					bgGrStart2.slideDown();bgGrEnd2.slideDown();bgColor2.slideDown();bgGrDeg2.slideDown();
				}else if((chosed === 'image')){
					bgImg2.slideDown();
					bgGrStart2.slideUp();bgGrEnd2.slideUp();bgColor2.slideUp();bgGrDeg2.slideUp();
				}
			});
			flipBgType2.triggerHandler("change");
		},1);
	};
    ...
    DdHelper.handleElementWidthAndOrder = function( PublishRequest ) {
        ...
        for ( var z = 0; z < editorElements.length; z++ ) {
            ...
            if ( elementId ) {
                ...
                if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 ) {
        ...
        for ( var i = 0; i < tl_editorElements.length; i++ ) {
            ...
            if ( elementId ) {
                ...
                if ( floated_width > 1 ) {
                    if ( typeof editorElement_2 == 'object' && editorElement_2.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && editorElement_2.className.indexOf( 'fotofly_fn_full_width' ) == -1 ) { // if an element behind exists, is a layout column but not a full width container
                        element_2.get( 'subElements' )[0].value = 'yes';
                        floated_width = parseFloat( editorElement.getAttribute( 'data-floated_width' ) );
                    }
                    if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && typeof editorElement_3 != 'object' && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 ) { // if current element is a layout column but not a full width container, forward element doesn't exist.
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    }
                    if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 && typeof editorElement_3 == 'object' && editorElement_3.className.indexOf( 'fotofly_fn_full_width' ) > -1 ) { // if current element is a layout colum but not full width and not element is full width column
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    }
                    if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && editorElement.className.indexOf( 'grid_one' ) > -1 ) { // if current element is a one column
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    }
                } else if ( floated_width == 1 ) {
                    if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 ) { // if current element is layout column and not a full width container
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    }
                    if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 && typeof editorElement_3 == 'object' && editorElement_3.className.indexOf( 'fotofly_fn_full_width' ) > -1 ) { // if current element is a layout colum but not full width and not element is full width column
                    ...
                    if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 && (typeof editorElement_3 != 'object' && (typeof editorElement_3 == 'object' && editorElement_3.className.indexOf( 'fotofly_fn_layout_column' ) == -1)) ) { // if current element is a layout column and not full width and forward element element doesn't exist, but if forward element is an object, it is not a layout column
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    } else if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && typeof editorElement_3 != 'object' && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 ) { // if current element is a layout column but not full width and forward element is not an object
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    } else if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && typeof editorElement_3 == 'object' && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 && editorElement_3.className.indexOf( 'fotofly_fn_layout_column' ) == -1 ) { // if current element is a layout column but not full width and forward element exists but is not a layout column
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    } else if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 && typeof editorElement_3 == 'object' && editorElement_3.className.indexOf( 'fotofly_fn_full_width' ) > -1 ) { // if current element is a layout colum but not full width and not element is full width column
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    } else {
                        if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 ) { // if current element is a layout column and not full width
                            element.get( 'subElements' )[0].value = 'no';
                        } else {
                            floated_width = 0;
                        }
            ...
            if ( ( i + 1 ) != tl_editorElements.length ) { //so this is not last
                ...
                if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && editorElement.className.indexOf( 'fotofly_fn_full_width' ) == -1 && tl_editorElements[i + 1].className.indexOf( 'fotofly_fn_element_box' ) > -1 ) {
                    element.get( 'subElements' )[0].value = 'yes';
                }
            }
        }
        ...
        var full_width_elements = document.querySelectorAll( '#editor > .fotofly_fn_full_width' );
            ...
            for ( var j = 0; j < tl_editorElements.length; j++ ) {
                ...
                if ( elementId ) {
                    ...
                    if ( ( j + 1 ) != tl_editorElements.length ) { //so this is not last
                        //layout column and next is builder element
                        if ( editorElement.className.indexOf( 'fotofly_fn_layout_column' ) > -1 && tl_editorElements[j + 1].className.indexOf( 'fotofly_fn_element_box' ) > -1 ) {
                            element.get( 'subElements' )[0].value = 'yes';
                        }
                    }
                }
    ...
    DdHelper.shortCodestoBuilderElements = function() {
        ...
        if ( window.fotofly_fn_builder_initial_load ) {
            content = $( '.frenify-builder-pagecontent' ).val();
            window.fotofly_fn_builder_initial_load = false;
        } else {
            content = $( '#content' ).val();
        }
        ...
        if ( content.length > 0 ) {
            ...
            $.post(
                ajaxurl, data, function( response ) {
                    ...
                    window.fotofly_fn_builder_fully_loaded = true;
                }
            );
        } else {
            window.fotofly_fn_builder_fully_loaded = true;
    ...
    DdHelper.loadCustomTabs = function() {
        instance = jQuery( '#frenify-page-builder' ).attr( 'instance' ); //post/page ID
        var data = {
    ...
    DdHelper.showHideLoader = function( state, message ) {
        ...
        if ( state == "show" ) {
            ...
            jQuery( '#fotofly_fn_loading_section' ).show();
        } else if ( state == "hide" ) {
            jQuery( '#fotofly_fn_loading_section' ).hide();
        }
    }
    /**
    ...
    DdHelper.RemoveDuplicates = function( refView ) {
        ...
        if ( $( "#" + prentId ).hasClass( "fotofly_fn_full_width" ) ) {
            app.editor.reRender();
        }
    }
