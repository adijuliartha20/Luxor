...
$minithumbperpage = 10;
if(isset($fotofly_fn_option['gallery_minithumbs_per'])){
	$minithumbperpage = $fotofly_fn_option['gallery_minithumbs_per'];
}

if(isset($fotofly_fn_option['gallery_mt_grid_type'])){
	$gallerygridtype = $fotofly_fn_option['gallery_mt_grid_type'];
}else{
	$gallerygridtype = 'square';
}


if(isset($_GET['temp'])){$gallerytemplate 	= $_GET['temp'];}
if(isset($_GET['per'])){$minithumbperpage 	= $_GET['per'];}
if(isset($_GET['grid'])){$gallerygridtype 	= $_GET['grid'];}


if(	$gallerygridtype == 'portrait'){
	$gallerythumb = fotofly_fn_callback_thumbs(122,150);
}else{$gallerythumb = '';}
?>
			...
			<div class="minithumbs">
				...
				<?php 
					...
					if(function_exists('rwmb_meta'))
					{
						...
						if($minithumbperpage > $count){
							$fn_count = $count;
						}else{
							$fn_count = $minithumbperpage;
						}
						
						if($fotofly_fn_gallery_images){
							foreach(array_slice($fotofly_fn_gallery_images, 0, $fn_count) as $img){
								$objectFull = wp_get_attachment_image_src( $img['ID'], 'full' );
								$object = wp_get_attachment_image_src( $img['ID'], 'fotofly_fn_thumb-300-300' );
								$object2 = wp_get_attachment_image_src( $img['ID'], 'fotofly_fn_thumb-720-9999' );
								$image_extraURL = $object[0];
								$image_extra = '<img class="noimg" src="'.$image_extraURL.'" alt="" />';
								if($gallerygridtype == 'portrait'){
									$image_overlay 	= '<div class="img_overlay" data-fn-bg-img="'.esc_url($image_extraURL).'"></div>';
								}else if($gallerygridtype == 'autow'){
									$image_overlay 	= '';
									$image_extra	= '<img class="noimg" src="'.$object2[0].'" alt="" />';
								}else{
									$image_overlay 	= '';
								}
								$children .= '<li class="lightbox" data-src="'.$objectFull[0].'"><div>'.$image_extra.$gallerythumb.$image_overlay.'</div></li>';
							}
						}
						$remainder 		= $count - $fn_count;
						$svgURL			= get_template_directory_uri() .'/framework/img/svg/gallery.svg';
						$gallerysvg 	= '<img class="fotofly_fn_svg" src="'.$svgURL.'" alt="" />';
						
						if(post_password_required($post)){
							$span	= '<span class="count">'.$count.'</span>';
							$children  	= '<li class="locked"><div>'.$image_extra.$password.$span.$gallerythumb.'</div></li>';
						}
						$remainderHTML 	= '<span class="count">+'.$remainder.'</span>';
						
						if($remainder > 0 && !post_password_required($post)){
							$children  .= '<li class="last"><div><a href="'.get_the_permalink().'"></a>'.$image_extra.$gallerysvg.$remainderHTML.$gallerythumb.'</div></li>';
						}
					}
					

				?>
				<div class="minithumb frenify_fn_lightbox">
						
						<?php echo wp_kses_post($fotofly_fn_gallery_post_title);?>
						
						<ul class="fn_<?php echo esc_attr($gallerygridtype);?>"><?php echo wp_kses_post($children);?></ul>
				</div>
				<?php endwhile; endif;?>
