...
class frenifySC_Slider {
	...
	public function __construct() {
		...
		add_filter( 'fotofly_fn_attr_slider-shortcode', array( $this, 'attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		$html = sprintf( '<div %s><ul %s>%s</ul></div>', frenifyCore_Plugin::attributes( 'slider-shortcode' ), frenifyCore_Plugin::attributes( 'slides' ), do_shortcode( $content ) );
	...
	function attr() {
		...
		$attr['class'] = 'frenify-slider-sc flexslider';
...
//		}
	...
	function render_child( $args, $content = '') {
		...
		if( $type == 'image' ) {
			...
			if( ! empty( $link ) && $link ) {
				$image_id = frenifyCore_Plugin::get_attachment_id_from_url( $link );
			} else {
				$image_id = frenifyCore_Plugin::get_attachment_id_from_url( $src );
			}
		...
		$html = sprintf( '<li %s>', frenifyCore_Plugin::attributes( 'slider-shortcode-slide-li' ) );

		if( $link && 
			! empty( $link )
		) {
			$html .= sprintf( '<a %s>', frenifyCore_Plugin::attributes( 'slider-shortcode-slide-link' ) );
		}
		
		if( ! empty( $type ) && 
			$type == 'video'
		) {
			$html .= sprintf( '<div %s>%s</div>', frenifyCore_Plugin::attributes( 'full-video' ), do_shortcode($content) );
		} else {
			$html .= sprintf( '<span %s><img %s /></span>', frenifyCore_Plugin::attributes( 'slider-shortcode-slide-img-wrapper' ), frenifyCore_Plugin::attributes( 'slider-shortcode-slide-img' ) );
		}
	...
	function slide_link_attr() {
		...
		$image_id = frenifyCore_Plugin::get_attachment_id_from_url( self::$child_args['link'] );
