...
abstract class DDElementTemplate {
				...
				for ($i = 0; $i < $elementsCount; $i++) {
					...
					foreach ($element['elements'] as $dynamic_element ) {
						...
						$output 	.= "<div class='clearfix form-element-container 
										frenify-element-child'><div class='name-description'>";
					...
					$output 	.="<a class='child-clone-row-remove frenify-shortcodes-button' 
									href='JavaScript:void(0)'>".__('Remove', 'frenify-core')."</a>";
					$output 	.= "</td></tr>";
				}
				$output 		.= "<tr><td><a id='frenify-child-add' href='JavaScript:void(0)'>".$element['buttonText']."</a>
									</td></tr>";
				$output 		.= "</table>";
	...
	private function parseElementType( $element ) {
		...
		switch ($element['type']) {
			...
			case ElementTypeEnum::COLOR:
				$output .= '<input type="text" class="text-field frenify-color-field" value="'.nl2br($element['value']).'" id="'.$element['id'].'" name="'.$element['id'].'" size="50"/>';
			break;
			
			case ElementTypeEnum::GALLERY:
				$output .= '<a href="'.$element['id'].'" class="frenify-gallery-button frenify-shortcodes-button">'.__('Attach Images to Gallery', 'frenify-core').'</a>';
			break;
			
			case ElementTypeEnum::UPLOAD:
				$button_class = ( $element['value'] == 'frenify-hidden-img' ? '' 		: 'remove-image' ); 
				$button_text  = ( $element['value'] == 'frenify-hidden-img' ? __('Upload', 'frenify-core') 	: __('Remove', 'frenify-core') ); 
				$output .= '<div class="frenify-upload-container">';
				$output .= '<img src="' . $element['value'] . '" alt="Image" class="uploaded-image" />';
				$output .= '<input type="hidden" class="frenify-form-text frenify-form-upload frenify-input" 
							name="' . $element['id'] . '" id="' . $element['id'] . '" value="' . $element['value'] . '" />' . "\n";
				$output .= '<a href="' . $element['id'] . '" class="frenify-upload-button '.$button_class.'" 
			...
			case ElementTypeEnum::ICON_BOX:
				$iconsArray = frenifyHelper::GET_ICONS_LIST();
	...
	*/
	public function create_visual_editor( $params ) {
		...
		$innerHtml.= "<div class='frenify-element-label'>".$this->config['name']."</div>";
