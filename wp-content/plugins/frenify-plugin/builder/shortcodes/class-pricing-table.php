...
class frenifySC_PricingTable {
	...
	public function __construct() {
		...
		add_filter( 'fotofly_fn_attr_pricingtable-shortcode', array( $this, 'attr' ) );
		add_filter( 'fotofly_fn_attr_pricingtable-shortcode-column-wrapper', array( $this, 'column_wrapper_attr' ) );	
		add_filter( 'fotofly_fn_attr_pricingtable-shortcode-price', array( $this, 'price_attr' ) );
		add_filter( 'fotofly_fn_attr_pricingtable-shortcode-row', array( $this, 'row_attr' ) );
		add_filter( 'fotofly_fn_attr_pricingtable-shortcode-footer', array( $this, 'footer_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		$defaults = frenifyCore_Plugin::set_shortcode_defaults(
			array(
				'class' 			=> '',
				'id' 				=> '',
				'backgroundcolor' 	=> $fotofly_fn_option['pricing_bg_color'],
				'bordercolor' 		=> $fotofly_fn_option['pricing_border_color'],
				'columns'			=> '',
				'dividercolor' 		=> $fotofly_fn_option['pricing_divider_color'],
				'type'				=> '1',
			), $args
		);
		...
		.pricing-table-{$this->pricing_table_counter} .frenify-panel, .pricing-table-{$this->pricing_table_counter} .panel-wrapper:last-child .frenify-panel, 
		.pricing-table-{$this->pricing_table_counter} .standout .frenify-panel, .pricing-table-{$this->pricing_table_counter}  .panel-heading, 
		...
		$html = sprintf( '%s<div %s>%s</div>', $styles, frenifyCore_Plugin::attributes( 'pricingtable-shortcode' ), do_shortcode($content) );
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'frenify-pricing-table pricing-table-%s %s-boxed-pricing row frenify-columns-%s columns-%s frenify-clearfix', 
	...
	function render_child_column( $args, $content = '') {
		...
		$defaults = frenifyCore_Plugin::set_shortcode_defaults(
			array(
				'class' 	=> 'frenify-pricingtable-column',
				'id' 		=> '',			
				'standout'	=> 'no',
				'title'		=> '',
			), $args
		...
		$html = sprintf( '<div %s><div %s><div %s><div %s><h3 %s>%s</h3></div>%s', frenifyCore_Plugin::attributes( 'pricingtable-shortcode-column-wrapper' ), 
						 frenifyCore_Plugin::attributes( 'panel-container' ), frenifyCore_Plugin::attributes( 'frenify-panel' ), frenifyCore_Plugin::attributes( 'panel-heading' ), 
						 frenifyCore_Plugin::attributes( 'title-row' ), $title, do_shortcode( $content ) );
	...
	function column_wrapper_attr() {
		...
		$attr['class'] = 'panel-wrapper frenify-column column col-lg-' . $columns . ' col-md-' . $columns . ' col-sm-' . $columns;
		
		if( self::$parent_args['columns'] == '5'  ) {
			$attr['class'] = 'panel-wrapper frenify-column column col-lg-2 col-md-2 col-sm-2';
		}
...
//		}
	...
	function render_child_price( $args, $content = '') {
		...
		if( isset( $price ) 
			...
			if( $currency_position != 'right' ){
				$pricing = sprintf( '<span %s>%s</span>', frenifyCore_Plugin::attributes( 'currency' ), $currency );
			}
			
			$pricing .= sprintf( '<span %s>%s</span>', frenifyCore_Plugin::attributes( 'integer-part' ), $price[0] );			
			
			if( array_key_exists( '1', $price ) ) {
				$pricing .= sprintf( '<sup %s>%s</sup>', frenifyCore_Plugin::attributes( 'decimal-part' ), $price[1] );
			}
			...
			if( $currency_position == 'right' ){
				...
				$pricing .= sprintf( '<span %s>%s</span>', frenifyCore_Plugin::attributes( $currency_classes ), $currency );
				
				if( $time ) {
					$pricing .= sprintf( '<span %s>%s</span>', frenifyCore_Plugin::attributes( $time_classes ), $time );
				}				
			}			
			...
			$html = sprintf( '<div %s><div %s>%s</div></div>%s', frenifyCore_Plugin::attributes( 'panel-body pricing-row' ), frenifyCore_Plugin::attributes( 'price ' . $pricing_class ), 
							  $pricing, do_shortcode( $content ) );
			
		} else {
		
			$html = sprintf( '<div %s></div>%s', frenifyCore_Plugin::attributes( 'panel-body pricing-row' ), do_shortcode( $content ) );
	...
	function render_child_row( $args, $content = '') {
		...
		if( $this->is_first_row ) {
			$html = sprintf( '<ul %s>', frenifyCore_Plugin::attributes( 'list-group' ) );
			$this->is_first_row = false;
		}	
		
		$html .= sprintf( '<li %s>%s</li>', frenifyCore_Plugin::attributes( 'list-group-item normal-row' ), do_shortcode( $content ) );
	...
	function render_child_footer( $args, $content = '') {
		...
		$html = sprintf( '</ul><div %s>%s</div>', frenifyCore_Plugin::attributes( 'panel-footer footer-row' ), do_shortcode( $content ) );
