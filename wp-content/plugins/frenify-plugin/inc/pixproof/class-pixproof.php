...
class PixProofPlugin {
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( $this->plugin_slug . '-plugin-script', 'pixproof', array(
			...
			'l10n' => array(
				'select' => esc_html__('Select', 'frenify-core'),
				'deselect' => esc_html__('Deselect', 'frenify-core'),
				'ofCounter' => esc_html__('of', 'frenify-core'),
				'next' => esc_html__('Next', 'frenify-core'),
				'previous' => esc_html__('Previous', 'frenify-core')
			)
		) );
	}
	...
	function add_plugin_admin_menu() {
		...
		$this->plugin_screen_hook_suffix = add_options_page( esc_html__( 'Proofing Settings', 'frenify-core' ), esc_html__( 'Proofing Settings', 'frenify-core' ), 'edit_plugins', $this->plugin_slug, array(
			$this,
	...
	function add_action_links( $links ) {
		return array_merge( array( 'settings' => '<a href="' . admin_url( 'options-general.php?page=pixproof' ) . '">' . esc_html__( 'Settings', 'frenify-core' ) . '</a>' ), $links );
	}
	...
	function hook_into_the_content( $content ) {
		if ( get_post_type() !== 'fotofly-fn-proofing' || post_password_required() ) {
			return $content;
		}
		$style = '';
		...
		if ( isset( self::$plugin_settings['gallery_position_in_content'] ) && ! empty( self::$plugin_settings['gallery_position_in_content'] ) ) {
			...
			if ( $gallery_position === 'before' ) {
				return $pixproof_output .'<div class="fotofly_fn_proof_content"><div>'. $content.'</div></div>';
			} else {
				return '<div class="fotofly_fn_proof_content"><div>'.$content.'</div></div>' . $pixproof_output;
			}
		}
	...
	function parse_comments( $comment = '' ) {
		...
		global $post;
		if ( 'fotofly-fn-proofing' !== $post->post_type ) {
			return $comment;
		}
	...
	public function generate_photos_zip_file() {
		...
		if ( post_password_required( $post ) ) {
			wp_send_json_error( esc_html__('The gallery password is required', 'frenify-core') );
		}
		...
		// quit if there is no gallery data
		if ( empty( $gallery_data ) || ! isset( $gallery_data[ 'gallery' ] ) ) {
			wp_send_json_error( esc_html__('No gallery data', 'frenify-core') );
		}

		$gallery_ids = explode( ',', $gallery_data[ 'gallery' ] );
		if ( empty( $gallery_ids ) ) {
			wp_send_json_error( esc_html__('Empty gallery', 'frenify-core') );
		}
